...
class DashboardController extends Controller {
    ...
    public function index() {
        ...
        $data['interest'] = DB::table('interest')->where(['status' => 1])->select('*')->orderBy('name', 'ASC')->get();
            ...
            $output .= '
				<div class="row PromotionDetail">
					<div class="col-md-8 col-sm-12 PromotionImg position-relative">
					  <img class="w-100" src="' . @$imageUrl . '" alt="">
					  <div class="ProductIconContainer">
						<a href="javascript:void(0)" class="edit-product" data-bs-toggle="modal" data-bs-target="#EditProductModal" relid="' . @$productInfo->id . '">
            ...
            $output .= '
				<div class="row PromotionDetail">
					<div class="col-md-8 col-sm-12 PromotionImg position-relative">
					  <img class="w-100" src="' . @$imageUrl . '" alt="">
					  <div class="ProductIconContainer">
						<a href="javascript:void(0)" class="edit-services" relid="' . @$serviceInfo->id . '" data-bs-toggle="modal" data-bs-target="#editServiceModel">
						  <i class="fa fa-pencil-square" aria-hidden="true"></i>
						</a>
						<a href="javascript:void(0)" data-bs-toggle="modal" data-bs-target="#DeleteProductModal">
						  <i class="fa fa-trash" aria-hidden="true"></i>
						</a>
					  </div>
					</div>
            ...
            if (!empty(@$userInfo)) {
                ...
                if (!empty(@$userInfo->area_interest)) {
                    ...
                    foreach ($interestId as $interestKey => $interestVal) {
                        $interest = DB::table('interest')->where(['id' => @$interestVal])->select('*')->orderBy('id', 'ASC')->first();
                        $interestConcate[] = $interest->id;
                    }
                }
    ...
    public function updateProdule(Request $request) {
        ...
        if (@$request->area_interest) {
            @$area_interest = implode(',', (array) @$request->area_interest);
        } else {
            @$area_interest = '';
        }
        if (@$request->dob) {
    ...
    public function referInvite(Request $request)
    {
        ...
        if ($result) {
            ...
            $response['status'] = 1;
            $response['msg'] = 'You have been successfully refer.';
    ...
    public function get_invitee_user_model(Request $request)
    {
        ...
        $newre = '<input class="form-control mt-4 mb-4 SearchBarInner" type="search" placeholder="Search"
                    aria-label="Search">

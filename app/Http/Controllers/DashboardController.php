...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $Sql = "SELECT * FROM promotion WHERE status = '1' AND user_id = '" . session()->get('USERLOGINID') . "' order by id desc";
        ...
        $data['interest'] = DB::table('interest')->select('*')->orderBy('name', 'ASC')->get();
        ...
        $favBusiness = "SELECT listing.* FROM listing INNER JOIN favouritebusiness ON listing.id = favouritebusiness.listing_id WHERE favouritebusiness.user_id='" . session()->get('USERLOGINID') . "'";
        $data['favBusiness'] = DB::select($favBusiness);
        ...
        $Sql = "SELECT first_name, last_name, id FROM users WHERE status = '1' AND user_type = '4' order by id DESC";
        ...
        $data['plan'] = DB::table('sub_plan')->where(['status' => 1, 'user_type' => 3])->select('*')->get();
        $data['adsPlan'] = DB::table('ads_sub_plan')->where(['status' => 1])->select('*')->get();
        $setting = DB::table('settings')->select('*')->first();
        /*$ip = $_SERVER["REMOTE_ADDR"];
        $query = @unserialize(file_get_contents('http://ip-api.com/php/'.$ip));
        $allBu = "SELECT *, (6371 * acos(cos(radians(" . $query['lat'] . ")) * cos(radians(`latitude`)) * cos(radians(`longitude`) - radians(" . $query['lon'] . ")) + sin(radians(" . $query['lat'] . ")) * sin(radians(`latitude`)))) as distance FROM listing having distance < 2000";
        ...
        $favEvent = "SELECT events.* FROM events INNER JOIN favouriteevent ON events.id = favouriteevent.event_id WHERE favouriteevent.user_id=" . session()->get('USERLOGINID') . "";
        $data['favEvent'] = DB::select($favEvent);
    ...
    public function addEvent(Request $request)
    {
        ...
        $data = ['event_name' => @$request->event_name, 'description' => @$request->event_description, 'category' => @$request->event_category, 'user_id' => @$request->userId, 'start_date' => @$start_date, 'end_time' => @$end_time, 'start_time' => @$start_time, 'location' => @$request->event_address, 'country' => @$request->event_country, 'state' => @$request->event_state, 'city' => @$request->event_city, 'latitude' => @$request->event_latitude, 'longitude' => @$request->event_longitude, 'zipcode' => @$request->event_zipcode, 'status' => 1, 'email' => @$request->event_email, 'phone' => @$request->event_phone, 'website' => @$request->event_website, 'tags' => @$tags, 'created_at' => date('Y-m-d H:i:s')];
    ...
    public function saveAds(Request $request)
    {
        ...
        if (@$request->age) {
            $age = implode(',', (array) @$request->age);
        } else {
            $age = '';
        }
        if (@$request->parental_status) {
        ...
        if ($result) {
            DB::table('users')->where(['id' => session()->get('USERLOGINID')])->update(['promotionCount' => DB::raw('promotionCount-1')]);
            $response['status'] = 1;
    ...
    public function get_promotion_detail(Request $request)
    {
        ...
        if (@$request->promotionId) {
            ...
            if ($promotionInfo->file_type == 1) {
                ...
                $file = '<video class="w-100" src="' . @$imageUrl . '" controls></video>';
            }
    ...
    public function get_promotion_edit_detail(Request $request)
    {
        $promotionInfo12 = [];
        if (@$request->promotionId) {
            $promotionInfo12 = DB::table('promotion')->where(['status' => 1, 'id' => @$request->promotionId])->select('*')->first();
            if (@$promotionInfo12->age_range) {
                $exTags = explode(",", @$promotionInfo12->age_range);
                foreach ($exTags as $k => $v) {
                    $promotionInfo123[] = $v;
                }
            }
        }
        $promotionInfo12->age_range = $promotionInfo123;
        //print_r($promotionInfo12);die;
        //$promotionInfo12['age_range'] = $promotionInfo_1;
        echo json_encode($promotionInfo12);
    }
    ...
    public function get_event_edit_detail(Request $request)
    {
        ...
        if (@$request->eventId) {
            ...
            $response['start_time'] = date('H:i:s', strtotime($eventInfo->start_time));
            $response['end_time'] = date('H:i:s', strtotime($eventInfo->end_time));
            if (@$eventInfo->tags) {
    ...
    public function updateEvent(Request $request)
    {
        ...
        $endTime = $request->edit_event_end_time;
        $end_time = date('H:i:s', strtotime($endTime));
        $startTime = $request->edit_event_start_time;
        $start_time = date('H:i:s', strtotime($startTime));
        $data = ['event_name' => @$request->event_name, 'description' => @$request->event_description, 'category' => @$request->event_category, 'start_date' => @$start_date, 'end_time' => @$end_time, 'start_time' => @$start_time, 'location' => @$request->event_address, 'country' => @$request->event_country, 'state' => @$request->event_state, 'city' => @$request->event_city, 'latitude' => @$request->event_latitude, 'longitude' => @$request->event_longitude, 'zipcode' => @$request->event_zipcode, 'email' => @$request->event_email, 'phone' => @$request->event_phone, 'website' => @$request->event_website, 'tags' => @$tags, 'updated_at' => date('Y-m-d H:i:s')];
        //$result = DB::table('events')->insertGetId($data);
                ...
                foreach ($subList as $k => $v) {
                    ...
                    $paymentList .= '
						<div class="col-lg-4 col-md-4 ps-0 mb-4">
						  <div class="TransactionBlock" style="height:100px !important">
							<div class="TransactionData">
							  <img src="' . url('assets/home/images/CompleteIcon.png') . '" alt="">
							  <div class="TransactionTextdata" style="display: flex;flex-direction: column;align-items: stretch;justify-content: center;flex-wrap: nowrap;">
    ...
    public function get_business_detail(Request $request)
    {
        ...
        if (@$request->businessId) {
            ...
            if (count($productList) > 0) {
                foreach ($productList as $k => $v) {
                    ...
                    $productOutPut .= '
						...
						style="cursor: pointer;" hide-edit-delete-product="' . @$request->hide_edit . '">
            ...
            if (count($serviceList) > 0) {
                foreach ($serviceList as $k => $v) {
                    ...
                    $serviceOutPut .= '
                        ...
                        style="cursor: pointer;" hide-edit-delete-service="' . @$request->hide_edit . '">
            ...
            if (!empty($listingInfo->qrpath)) {
                $qrpath = url('' . $listingInfo->qrpath . '');
            } else {
                $qrpath = url('assets/home/images/Icon27.png');
            }
            $output = '<div class="row PromotionDetail">
    ...
    public function get_business_edit_detail(Request $request)
    {
        ...
        if (@$request->businessId) {
            ...
            if (@$businessInfo->tags) {
                $exTags = explode(",", @$businessInfo->tags);
                foreach ($exTags as $k => $v) {
                    $tagsArray[] = $v;
                }
            }
    ...
    public function delete_business($id)
    {
        ...
        if ($result) {
            DB::table('product')->where('listing_id', @$id)->delete();
            DB::table('services')->where('listing_id', @$id)->delete();
    ...
    public function addProductCat(Request $request)
    {
        ...
        if ($request->cat_name) {
            ...
            if ($count > 0) {
                ...
                $data = ['name' => @$request->cat_name, 'user_id' => session()->get('USERLOGINID'), 'status' => 1, 'created_at' => date('Y-m-d H:i:s')];
    ...
    public function accept_invitation(Request $request)
    {
        $response = [];
        if (@$request->invi_id) {
            if (@$request->accept_comment) {
                $comment = @$request->accept_comment;
            } else {
                $comment = '';
            }
            $data = ['status' => '1', 'comment' => @$comment, 'updated_at' => date("Y-m-d H:i:s")];
    ...
    public function reject_invitation(Request $request)
    {
        ...
        if (@$request->invi_id) {
            ...
            $data = ['status' => '0', 'comment' => @$comment, 'updated_at' => date("Y-m-d H:i:s")];
            $result = DB::table('invitation')->where(['id' => @$request->invi_id])->update(@$data);
            if ($result) {
    ...
    public function contact_invitee(Request $request)
    {
        $response = [];
        if (@$request->contact_invitee_receiver_id) {
            if (@$request->message_contact_invitee) {
                $comment = @$request->message_contact_invitee;
            } else {
                $comment = '';
            }
            $notiData = ['noti_msg' => @$comment, 'sender_id' => session()->get('USERLOGINID'), 'receiver_id' => @$request->contact_invitee_receiver_id, 'type' => 5, 'status' => '1', 'created_at' => date("Y-m-d H:i:s")];
            $result = DB::table('notifications')->insertGetId($notiData);
            if ($result) {
                $response['status'] = 1;
                $response['msg'] = 'Successfully sent your message.';
            } else {
                $response['status'] = 0;
                $response['msg'] = 'Some error occure, Please try again.';
            }
    ...
    {
        $response = [];
        if (@$request->message_contact_admin) {
            $comment = @$request->message_contact_admin;
        } else {
            $comment = '';
        }
        $notiData = ['message' => @$comment, 'sender_id' => session()->get('USERLOGINID'), 'created_at' => date("Y-m-d H:i:s")];
    ...
    public function promotion_payment()
    {
        ...
        if (empty(@$_GET['planId'])) {
            echo 'planId is required.';
            exit();
        }
        if (empty(@$_GET['adsId'])) {
            echo 'adsId is required.';
            exit();
        }
    ...
    function get_product_details(Request $request)
    {
        ...
        if (@$request->productId) {
            ...
            if (!empty($productInfo->qrpath)) {
                $qrpath = url('' . $productInfo->qrpath . '');
            } else {
                $qrpath = url('assets/home/images/Icon27.png');
            }
            $output .= '
    ...
    function get_service_details(Request $request)
    {
        ...
        if (@$request->serviceId) {
            ...
            if (!empty($serviceInfo->qrpath)) {
                $qrpath = url('' . $serviceInfo->qrpath . '');
            } else {
                $qrpath = url('assets/home/images/Icon27.png');
            }
            $output .= '
    ...
    function network()
    {
        ...
        //$data['allMem']  = DB::table('users')->where(['status' => 1, 'user_type' => 4, 'user_type'])->select('*')->get();
        $data['allMem'] = DB::table('users')->whereRaw("id != '" . session()->get('USERLOGINID') . "' AND (user_type = 3 OR user_type = 4) AND status = 1")->limit(15)->select('*')->orderBy('id', 'ASC')->get();
        $data['countCreator'] = DB::table('users')->whereRaw("id != '" . session()->get('USERLOGINID') . "' AND (user_type = 3 OR user_type = 4) AND status = 1")->select('*')->count();
    ...
    public function stripe_connect()
    {
        ...
        $data['allMem'] = DB::table('users')->where(['status' => 1, 'user_type' => 4])->select('*')->get();
        //$data['myMem']  = DB::table('users')->where(['status' => 1, 'user_type' => 4])->select('*')->get();
        $Sql = "SELECT users.id as userId, users.first_name, users.last_name, users.profile_image, favouriteusers.fav_user_id, favouriteusers.user_id FROM users INNER JOIN favouriteusers ON users.id = favouriteusers.user_id WHERE users.status='1' AND users.id='" . session()->get('USERLOGINID') . "'";
        $data['myMem'] = DB::select($Sql);
        //print_r($data['myMem']);die;
    ...
    public function get_profile_info(Request $request)
    {
        ...
        if ($request->userId) {
            ...
            if (!empty(@$userInfo)) {
                ...
                $interestConcate = [];
                if (!empty(@$userInfo->area_interest)) {
                    $interestId = explode(',', @$userInfo->area_interest);
                    foreach ($interestId as $interestKey => $interestVal) {
                        $interests = DB::table('interest')->where(['id' => @$interestVal])->select('*')->orderBy('id', 'ASC')->first();
                        $interestConcate[] = $interests->id;
                    }
                }
                $response['interest'] = @$interestConcate;
            }
        }
        echo json_encode(@$response);
    }
    public function updateProdule(Request $request)
    {
        ...
        if (@$request->interest) {
            @$interest = implode(',', (array) @$request->interest);
        } else {
            @$interest = '';
        }
        if (@$request->dob) {
        ...
        $data = ['first_name' => @$firstName, 'last_name' => @$lastName, 'email' => @$email, 'phone' => @$phone, 'address' => @$location, 'country' => @$country, 'state' => @$state, 'latitude' => @$latitude, 'longitude' => @$longitude, 'city' => @$city, 'bio' => @$bio, 'tags' => @$tags, 'area_interest' => @$interest, 'dob' => @$dob, 'updated_at' => date('Y-m-d H:i:s')];
    ...
    public function transaction()
    {
        ...
        if (!empty(@$_GET['from_date']) && !empty(@$_GET['to_date'])) {
            $data['transactionList'] = DB::table('transaction')->whereRaw("DATE(created_at) BETWEEN '" . @$_GET['from_date'] . "' AND '" . @$_GET['to_date'] . "' AND user_id = " . session()->get('USERLOGINID') . "")->select('*')->get();
        } else {
    ...
    public function purchase_history()
    {
        ...
        if (!empty(@$_GET['from_date']) && !empty(@$_GET['to_date'])) {
            $data['purList'] = DB::table('transaction')->whereRaw("payment_type = 6 AND user_id = " . session()->get('USERLOGINID') . " AND DATE(created_at) BETWEEN '" . @$_GET['from_date'] . "' AND '" . @$_GET['to_date'] . "'")->select('*')->get();
        } else {
    ...
    public function load_network_data()
    {
        if (!empty($_GET['lastId'])) {
            ...
            //echo $lastId;die;
            $newre = '';
            $get_data = DB::table('users')->whereRaw("id != '" . session()->get('USERLOGINID') . "' AND (user_type = 3 OR user_type = 4) AND status = 1 AND id > " . $lastId . "")->limit(10)->select('*')->orderBy('id', 'ASC')->get();
            if (count($get_data) > 0) {
    ...
    public function saleList()
    {
        ...
        //$data['purList']  = DB::table('transaction')->where(['payment_type' => 6, 'user_id' => session()->get('USERLOGINID')])->select('*')->get();
        if (!empty(@$_GET['from_date']) && !empty(@$_GET['to_date'])) {
            $proList = DB::table('transaction')->whereRaw("payment_type = 6 AND DATE(created_at) BETWEEN '" . @$_GET['from_date'] . "' AND '" . @$_GET['to_date'] . "'")->select('*')->get();
        } else {
                ...
                if (is_array($productInfo) || is_object($productInfo)) {
                    foreach ($productInfo as $proKey => $proVal) {
                        if (@$proVal['specipication'] == 'service') {
                            $proList_1 = DB::table('services')->whereRaw("id = " . $proVal['product_id'] . " AND user_id = " . session()->get('USERLOGINID') . "")->select('*', DB::raw("'service' as type"))->orderBy('id', 'DESC')->first();
                        } else {
                            $proList_1 = DB::table('product')->whereRaw("id = " . $proVal['product_id'] . " AND user_id = " . session()->get('USERLOGINID') . "")->select('*', DB::raw("'product' as type"))->first();
                        }
                        if (!empty(@$proList_1)) {
    ...
    public function get_invitee_user_model(Request $request)
    {
                ...
                $allAthaleticUser .= '
					    <div class="col-lg-4 col-md-4 ps-0 mb-4">
                          <div class="AllContainBlock">
                            ...
                            <a href="javascript:void(0);" class="sendInvitationAgain" relid="' . @$v->id . '">
                              <img src="' . url('assets/home/images/Icon7.png') . '" alt="">
                            </a>
                          </div>
                        </div>
        ...
        $newre = '
                    <div class="allInvitedBlock tab-pane fade show active" id="AllTab" role="tabpanel" aria-labelledby="All-Tab">
                      <div class="row m-0 AllContain">
                        ' . @$allAthaleticUser . '
                      </div>
                    </div>
                    <div class="InvitedBlock tab-pane fade" id="InvitedPeopleTab" role="tabpanel"
    ...
    public function searchInvitePeople(Request $request)
    {
        ...
        if (count($invitation) > 0) {
            foreach ($invitation as $k => $v) {
                $get_receiverInfo = DB::table('repeat_invitation')->where(['invitation_id' => @$v->id])->select('*')->first();
                if (!empty($get_receiverInfo)) {
                    if (!empty($get_receiverInfo->receiver_id)) {
                        $inviteeId[] = $get_receiverInfo->receiver_id;
                    }
                }
            }
        ...
        $allAthaleticUser = '';
        $allAthaletic = DB::table('users')->whereRaw("status = 1 AND user_type = 4 AND CONCAT(first_name, ',', last_name) LIKE '%$request->searchText%'")->select('*')->orderBy('id', 'DESC')->get();
        if (count($allAthaletic) > 0) {
            foreach ($allAthaletic as $k => $v) {
                if (!empty(@$v->profile_image) && file_exists('public/profile/' . @$v->profile_image . '')) {
                    $profile_2 = url('profile/' . @$v->profile_image . '');
    ...
    function downloadCsvAll()
    {
        ...
        if (!empty(@$_GET['from_date']) && !empty(@$_GET['to_date'])) {
            $data['list'] = DB::table('transaction')->whereRaw("DATE(created_at) BETWEEN '" . @$_GET['from_date'] . "' AND '" . @$_GET['to_date'] . "' AND user_id = " . session()->get('USERLOGINID') . "")->select('*')->get();
        } else {
            $data['list'] = DB::table('transaction')->where(['user_id' => session()->get('USERLOGINID')])->select('*')->get();
        }
        $array = [];
    ...
    function downloadPurchasedHistory()
    {
        $array = [];
        $filename = 'purchase_history_list' . date('Ymd') . '.csv';
        header("Content-Description: File Transfer");
        header("Content-Disposition: attachment; filename=$filename");
        header("Content-Type: application/csv; ");
        if (!empty(@$_GET['from_date']) && !empty(@$_GET['to_date'])) {
            $data['purList'] = $purList = DB::table('transaction')->whereRaw("payment_type = 6 AND user_id = " . session()->get('USERLOGINID') . " AND DATE(created_at) BETWEEN '" . @$_GET['from_date'] . "' AND '" . @$_GET['to_date'] . "'")->select('*')->get();
        } else {
            $data['purList'] = $purList = DB::table('transaction')->where(['payment_type' => 6, 'user_id' => session()->get('USERLOGINID')])->select('*')->get();
        }
        if (count(@$purList)) {
            foreach (@$purList as $k => $v) {
                if (@$v->product_info) {
                    ...
                    foreach ($product as $proKey => $proVal) {
                        if (@$proVal['specipication'] == 'service') {
                            $productInfo = DB::table('services')->where(['id' => $proVal['product_id']])->select('*')->orderBy('id', 'DESC')->first();
                            $proImg = DB::table('services_image')->where(['service_id' => $proVal['product_id']])->select('*')->orderBy('id', 'ASC')->first();
                            ...
                            $proImg = DB::table('product_image')->where(['product_id' => $proVal['product_id']])->select('*')->orderBy('id', 'ASC')->first();
                            if (!empty(@$proImg->image) && file_exists('public/product/' . @$proImg->image . '')) {
                                $productImg = url('product/' . @$proImg->image . '');
                            } else {
                                $productImg = url('noimage.jpg');
                        ...
                        if (!empty(@$productInfo)) {
                            //echo $productInfo->id;
                            $businessName = '--';
                            $listing = DB::table('listing')->where(['id' => $proVal['product_id']])->select('*')->orderBy('id', 'DESC')->first();
                            if ($listing) {
                            ...
                            $array[] = [
                                ...
                                'Amount' => "$" . @$proVal['price'],
                                'PurchasedOn' => date('M d, Y', strtotime($v->created_at)),
                            ];
                        }
                    }
                }
        ...
        $file = fopen('php://output', 'w');
        $header = array("productName", "TransactionId", "OrderId", "Business", "Amount", "PurchasedOn");
        fputcsv($file, $header);
        foreach ($array as $key => $line) {
            fputcsv($file, $line);
        }
    ...
    function downloadSalesList()
    {
        $array = [];
        $filename = 'Sales_list' . date('Ymd') . '.csv';
        header("Content-Description: File Transfer");
        header("Content-Disposition: attachment; filename=$filename");
        header("Content-Type: application/csv; ");
        $data['myProList'] = [];
        ...
        if (count(@$data['myProList'])) {
            foreach (@$data['myProList'] as $k => $v) {
                if (@$v->type == 'service') {
        ...
        $file = fopen('php://output', 'w');
        $header = array("productName", "Business", "Amount", "AdminShare", "SPShare", "SalesOn");
        fputcsv($file, $header);
        foreach ($array as $key => $line) {
            fputcsv($file, $line);
        }
    ...
    public function addRating(Request $request)
    {
        if (@$request->productId) {
            @$userId = session()->get('USERLOGINID');
            $checkUser = DB::table('business_review')->where(['product_id' => @$request->productId, 'user_id' => @$userId])->select('*')->orderBy('id', 'DESC')->first();
            if ($checkUser) {
                $response = ["status" => 0, "error" => 'Already added rating.'];
                return response()->json($response, 200);
                exit();
            }
            $data = array('user_id' => @$userId, 'product_id' => @$request->productId, 'rating' => @$request->rating, 'created_at' => date('Y-m-d H:i:s'));
            $result = DB::table('business_review')->insertGetId($data);
            if ($result) {
                $response = ["status" => 1, "msg" => 'Successfully added your rating.'];
                return response()->json($response, 200);
            } else {
                $response = ["status" => 0, "msg" => 'Some error occure, Please try again.'];
                return response()->json($response, 200);
            }
        } else {
            $response = ["status" => 0, "msg" => 'productId re required.'];
            return response()->json($response, 200);
        }
    }
    public function appearence_start(Request $request)
    {
        if (@$request->invitationId) {
            $data = ['status' => '4', 'updated_at' => date("Y-m-d H:i:s")];
            ...
            if ($result) {
                ...
                $response = ["status" => 1, "msg" => 'Appeareance start successfully.'];
                return response()->json($response, 200);
            } else {
                $response = ["status" => 0, "msg" => 'Some error occure, Please try again.'];
                return response()->json($response, 200);
            ...
            $response = ["status" => 0, "msg" => 'invitationId required.'];
            return response()->json($response, 200);
        }
    }
    public function appearence_ended(Request $request)
    {
        if (@$request->invitationId) {
            $data = ['status' => '5', 'updated_at' => date("Y-m-d H:i:s")];
            $result = DB::table('invitation')->where(['id' => $request->invitationId])->update(@$data);
            if ($result) {
                $inviInfo = DB::table('repeat_invitation')->where(['invitation_id' => $request->invitationId])->select('*')->orderBy('id', 'DESC')->first();
                $invitationInfo = DB::table('invitation')->where(['id' => $request->invitationId])->select('*')->orderBy('id', 'DESC')->first();
                ...
                $response = ["status" => 1, "msg" => 'Appeareance ended successfully.'];
                return response()->json($response, 200);
            } else {
                $response = ["status" => 0, "msg" => 'Some error occure, Please try again.'];
                return response()->json($response, 200);
    ...
    function deleteAccount()
    {
        ...
        $address = "Kathmandu, Nepal"; // Google HQ
        $prepAddr = str_replace(' ', '+', $address);
        $apiKey = 'AIzaSyAoL5Cb3GKL803gYag0jud6d3iPHFZmbuI'; // Google maps now requires an API key.

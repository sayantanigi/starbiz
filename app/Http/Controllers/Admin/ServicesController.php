...
class ServicesController extends Controller {
	...
	public function add()
    {
		...
		$data['listing'] = DB::table('listing')->select('*')->orderBy('business_name', 'ASC')->get();        $data['tags'] = DB::table('tags')->select('*')->orderBy('name', 'ASC')->get();
	...
	public function save(Request $request)
    {
       ...
       $product_tags = '';		if(@$request->tags){			$product_tags = implode(",", @$request->tags);		}
		$data = ['name' => $service_name, 'category' => $service_category, 'listing_id' => $service_listing, 'price' => $service_price, 'special_price' => @$service_special, 'description' => $service_description, 'tags' => $product_tags, 'status' => $service_status, 'user_id' => 0, 'created_at' => date('Y-m-d H:i:s')];
	...
	public function update(Request $request)
    {
        ...
        $product_tags = '';		if(@$request->tags){			$product_tags = implode(",", @$request->tags);		}
		$data = ['name' => $service_name, 'category' => $service_category, 'listing_id' => $service_listing, 'price' => $service_price, 'special_price' => @$service_special, 'description' => $service_description, 'status' => $service_status, 'tags' => $product_tags, 'user_id' => 0, 'updated_at' => date('Y-m-d H:i:s')];
		//print_r($data);die;

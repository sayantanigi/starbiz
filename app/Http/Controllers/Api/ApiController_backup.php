...
class ApiController_backup extends Controller {
	...
	public function uploadProfile_post(Request $request){
		...
		if(!$validator->fails()){
			if ($request->profilePhoto) {
				$pimg = $request->profilePhoto;
				$pextn = $pimg->getClientOriginalExtension();
				$ppath = public_path('profile/');
				$profile = rand() . '.' . $pextn;
				$pimg->move($ppath, $profile);
			} else {
			...
			if ($request->coverPhoto) {
				$cimg = $request->coverPhoto;
				$cextn = $cimg->getClientOriginalExtension();
				$cpath = public_path('profile/');
				$cover = rand() . '.' . $cextn;
				$cimg->move($cpath, $cover);
			} else {
    ...
    {
		$list = DB::table('users')->where(['status' => 1, 'user_type' => 10])->select('*')->orderBy('id', 'DESC')->get();
		if(count($list) > 0){
			foreach($list as $k => $v){
				if(!empty(@$v->profile_image) && file_exists('public/profile/'.@$v->profile_image.'')){
					$profilePic = url('profile/'.@$v->profile_image.'');
			...
			if(count($list) > 0){
				...
				foreach($list as $k => $v){
					...
					foreach($pro_data as $productKey => $productVal){
						$image_1  = DB::table('product_image')->where(['product_id' => @$productVal['product_id']])->select('*')->orderBy('id', 'DESC')->first();
						if(!empty($image_1)){
							if(!empty($image_1->image) && file_exists('public/product/'.$image_1->image.'')){
								$productImg = url('product/'.$image_1->image.'');
							}

...
class ApiController_20032025 extends Controller {
            ...
            if (count($checkAuth) == 1) {
                ...
                if (!empty($userSub)) {
                    ...
                    if (count($accessList) > 0) {
						...
						//print_r($arrayAcc);die;
						$i = 0;
                        foreach ($accessList as $accessKey => $accessVal) {
                            $accessMenu = DB::table('sub_access_menu')->where(['id' => @$accessVal->menu_id])->select('*')->first();
                            $menu = ['menuId' => $accessMenu->id, 'accessMenu' => $accessMenu->menu];
							//print_r($arrayAcc[$accessKey]);
                            // if (empty(@$accessVal->number_of)) {
                                // $number_of = 0;
                            // } else {
                                // $number_of = @$accessVal->number_of;
    ...
    public function addpromotionAds_post(Request $request)
    {
        ...
        if (!$validator->fails()) {
			...
			// print_r(@$request->ageRange);die;
			// $ageRange = !empty(@$request->ageRange) && is_array(@$request->ageRange) ? @$request->ageRange : [];
			// if(@$ageRange){
				// $ageRange1 = implode(",", @$ageRange);
			// }
			//print_r($ageRange1);die;
			$ageRange = [];
			if(@$request->ageRange){
				$ageRange = implode(",", @$request->ageRange);
			}
			//print_r($ageRange);die;
            @$radius = '';
            @$description = '';
        ...
        if (!empty(@$_GET['userId'])) {
            ...
            if ($userCheck > 0) {
                ...
                if (count($list) > 0) {
                    foreach ($list as $k => $v) {
                       //print_r($v);
					   // $eventInfo = "select event_name, location, start_date from events where id = '" . $v->event_id . "' AND DATE(start_date) > '" . date('Y-m-d') . "' LIMIT 1";
					   // echo $eventInfo;die;
	...
	public function AllInvitation_get(){
		$allAthaletic = DB::table('users')->whereRaw("status = 1 AND user_type = 10")->select('*')->orderBy('id', 'DESC')->get();

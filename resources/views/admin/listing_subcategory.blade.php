								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											<tr>
												<td><?= $key+1 ?></td>
												<?php
												    $cate = DB::table('listing_category')->where(['id' => @$v->listing_cat_id])->select('*')->orderBy('id', 'DESC')->first();
												?>
												<td><?= @$cate->name?></td>
												<td><?= @$v->name?></td>
												...
												<td class="text-center">
														...
														<a href="<?=url('admin/listing/edit-subcategory/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">

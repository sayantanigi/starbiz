                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=url('admin/discount/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
						@if (session('status'))
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											<tr>
												...
												<td><?= @$discount?></td>
												<td><?= @$v->coupon_code?></td>
												<td><?= date('M d, Y', strtotime(@$v->expire_on))?></td>
												...
												<td class="text-center">
												    <a href="<?=url('admin/discount/edit/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/discount/delete/') ?>/'+dealId
			}
		});
	}

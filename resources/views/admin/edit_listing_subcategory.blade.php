            ...
            <div class="row">
                ...
                <div class="col-lg-6 mb-3">
                  <div class="card shadow rounded">
                     <div class="card-body">    
                        <form action="{{url('admin/listing/update-subcategory')}}" method="post" enctype="multipart/form-data" >
                            @csrf
                            ...
                            <div class="form-group mb-2">
								...
								<select class="form-control" name="category"  id="category" required>
									...
									<?php 
									    if(!empty(@$cat)){
											foreach(@$cat as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$result->listing_cat_id == @$v->id) ? 'selected' : '').'>'.@$v->name.'</option>';
											}
										}
									?>
								</select>
							...
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Subcategory Name</label>
                                <input type="text" class="form-control" name="subcategory"  id="subcategory" value="<?=@$result->name?>" required>
								 ...
								 <div class="col-lg-12 mb-3">
									<div class="card rounded">
										<div class="card-body">
											...
											<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Subcategory Name</h6></label><p class="text-muted" id="subcategory-name"><?=@$result->name?></p></div>
                                            <?php
											    $status = '';
											    if(@$result->status == 1){
													$status = 'Active';
...
$(document).on('keyup','#subcategory',function(e){
	var pck_name = $(this).val();

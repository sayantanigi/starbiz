										...
										<?php foreach ($myProList as $k => $v): ?>
										    ...
										    <?php
												...
												if($v->user_id == 0){
													...
													$userInfo = DB::table('users')->where(['id' => $v->user_id])->select('first_name', 'last_name', 'email')->first();
													if($userInfo){
											...
											<tr>
												<td><?= $k+1 ?></td>
												<td><img src="<?=@$productImg?>" alt="" style="width: 72px;height: 61px;object-fit: unset;border-radius: 4px;"><br/><?=@$v->name?></td>
												<!--<td><?=@$v->name?></td>-->
												<td><?=@$listing->business_name?></td>
												<td><?=@$userName?></td>
												<td><?=@$v->purchasedUserName?></td>

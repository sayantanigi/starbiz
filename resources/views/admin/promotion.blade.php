<?php
	$promotionMenu_1 = DB::table('role_permission')->where(['role_id' => session()->get('ROLE_ID'), 'menu_id' => 13])->select('*')->orderBy('id', 'DESC')->first();
?>
               ...
               <div class="card custom-shadow rounded-lg border">
                  <div class="card-body">
                  	<div class="row">
	                    ...
	                    <?php if(@$promotionMenu_1->created == 1){?> 
							<div class="col-sm-2 text-end" style="padding-left: 54px;">
								<a href="<?=url('admin/promotion/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
							</div>
						<?php } ?>
										...
										<?php foreach ($result as $key => $v): ?>
											<tr>
												...
												<?php if(@$v->file_type == 1){ ?>
												    <td style="width:20%"><img src="<?=url('ads/'.@$v->image.'')?>" style="width:100%"></td>
												<?php }else{ ?>
												    <td style="width:20%"><video src="<?=url('ads/'.@$v->image.'')?>" style="width:100%" controls></video></td>
												<?php } ?>
												...
												<td>
												    <?php if(@$promotionMenu_1->edited == 1){?> 
														<a href="<?=url('admin/promotion/edit/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
															<i class="fas fa-edit"></i>
														</a>
													<?php } ?>
													
													<a href="<?=url('admin/promotion/view/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
															<i class="fas fa-eye"></i>
														</a>
													
													<?php if(@$promotionMenu_1->deleted == 1){?> 
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/promotion/delete/') ?>/'+dealId
			}
		});
	}

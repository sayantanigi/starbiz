                  ...
                  <div class="card-body">
                  	<div class="row">
						...
						<div class="col-sm-2 text-end" style="padding-left: 54px;">
							<a href="<?=url('admin/services/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
						</div>
	                </div>   	
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $k => $v): ?>
											<tr>
												...
												<td><?=@$v->name?></td>
												<?php
												    $listing = DB::table('listing')->select('business_name')->where(['id' => @$v->listing_id])->orderBy('id', 'DESC')->first();
													
													$category = DB::table('product_category')->select('*')->where(['id' => @$v->category])->orderBy('id', 'DESC')->first();
												?>
												...
												<td class="text-center">
													...
													<a href="<?= url('admin/services/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
														<i class="fas fa-edit"></i>
													</a>  
											   
													<a href="<?= url('admin/services/view/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
														<i class="fa fa-eye"></i>
													</a>
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/services/delete/') ?>/'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo url('admin/services/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				id: String(id),        
		...
		.done(function(data) {  
			...
			if(newStatus == 1){
				swal({title: "Sucess!", text: "<strong>service status is Activate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}else if(newStatus == 0){
				swal({title: "Sucess!", text: "<strong>service status is Inctivate</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = " "});
			}
		})
		.fail(function(data) {      
			console.log(data);       

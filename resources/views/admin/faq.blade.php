                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	 <a href="<?=url('admin/cms/add-faq')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add Faq</a>
	                     </div>
	                  </div>   	
                     <div class="">
					    @if (session('status'))
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											...
											<tr>
												...
												<td style="width:30%;"><?=@$v->question; ?></td>
												<td class="tableDes" style="width:30%;"><p class="comment more tableDes"><?=strip_tags(@$v->answer); ?></p></td>
												<td>
												...
												<td class="text-center">
												    <a href="<?= url('admin/cms/edit-faq/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/cms/delete-faq/') ?>/'+dealId
			}
		});
	}
	...
	function changePckStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo url('admin/cms/changestatus-faq'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				faqId: String(id),        

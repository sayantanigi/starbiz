<?php
$planMenu_1 = DB::table('role_permission')->where(['role_id' => session()->get('ROLE_ID'), 'menu_id' => 12])->select('*')->orderBy('id', 'DESC')->first();
?>
                    ...
                    <div class="card custom-shadow rounded-lg border">
                        <div class="card-body">
                            <div class="row">
                                ...
                                <?php if (@$planMenu_1->created == 1) { ?>
                                <div class="col-sm-2 text-end" style="padding-left: 54px;">
                                    <a href="<?=url('admin/subscription/add')?>" class="btn btn-success btn-sm"><i
                            ...
                            <div class="">
                                ...
                                <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
                                    ...
                                    <tbody class="text-center">
                                        ...
                                        if (!empty(@$v->user_type)) {
                                            $userType = DB::table('user_type')->where(['id' => @$v->user_type])->select('*')->first();
                                        ...
                                        <tr>
                                            ...
                                            ?>
                                            <td><?= @$v->duration?> &nbsp; <?=@$type?></td>
                                            <!--<td>
                                                <?php
                                                $output = '';
                                                $output .= '<ul>';
                                                if (@$v->access) {
                                                    $accessEx = explode(',', @$v->access);
                                                    foreach ($accessEx as $v1) {
                                                        $output .= '<li>' . $v1 . '</li>';
                                                    }
                                                }
                                                $output .= '<ul>';
                                            ...
                                            <td class="text-center">
                                                <?php if (@$planMenu_1->edited == 1) { ?>
                                                <a href="<?=url('admin/subscription/edit/' . @$v->id . '')?>"
                                ...
                                <!-- <div class="container mt-3">
								    <button type="button" class="btn btn-primary hide" data-bs-toggle="modal" data-bs-target="#dealModal">Open modal</button>
								</div> -->
                                <!-- The Modal -->
                                    ...
                                    <div class="modal-dialog modal-xl">
                                        <div class="modal-content" style="width: 28% !important; margin: 0 auto;">
                                            ...
                                            <div class="modal-footer"
                                                style="-webkit-box-align: center !important; justify-content: center !important;">
                                                <button type="button" class="btn btn-danger"
                                                    data-bs-dismiss="modal">Close</button>
                                                <button type="button" class="btn btn-primary"
                                                    onclick="myfunc()">Download</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script type="text/javascript">
        ...
        function deleteDeals(dealId) {
            swal({
                ...
                if (isConfirm) {
                    window.location.href = '<?= url('admin/subscription/delete/') ?>/' + dealId
                }
            });
        }

                  	...
                  	<div class="row">
	                  	<div class="col-sm-10">
							 ...
							 <div class="col-sm-2 text-end" style="padding-left: 54px;">
								 <a href="<?=url('admin/promotion/plan/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
							 </div>
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											<tr>
												...
												<td><?= @$v->plan_type?></td>
												<td><?= @$v->ads_type?></td>
												<td><?= (!empty(@$v->price) ? @$v->price : '--')?></td>
												<td><?= @$v->plan_duration?></td>
												...
												<td class="text-center">
														...
														<a href="<?=url('admin/promotion/plan/edit/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/promotion/plan/delete') ?>/'+dealId
			}
		});
	}

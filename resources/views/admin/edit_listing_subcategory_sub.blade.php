            ...
            <div class="row">
                ...
                <div class="col-lg-6 mb-3">
                  <div class="card shadow rounded">
                     <div class="card-body">    
                        <form action="{{url('admin/listing/update-subcategory-update')}}" method="post" enctype="multipart/form-data" >
                            @csrf
                            ...
                            <div class="form-group mb-2">
								...
								<select class="form-control" name="subcategory"  id="subcategory" required>
									...
									<?php 
									    if(!empty(@$sub)){
											foreach(@$sub as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$v->id == @$result->listing_subcategory_id) ? 'selected' : '').'>'.@$v->name.'</option>';
											}
										}
									?>
								</select>
							...
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Subcategory Sub Name</label>
                                <input type="text" class="form-control" name="subcategorysub"  id="subcategorysub" required value="<?=@$result->name?>">
								 ...
								 <div class="col-lg-12 mb-3">
									<div class="card rounded">
										<div class="card-body">
											...
											<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Subcategory Sub Name</h6></label><p class="text-muted" id="subcategory-name"></p></div>
...
$(document).on('keyup','#subcategorysub',function(e){
	var pck_name = $(this).val();
	
	if(pck_name){ 
	  $("#subcategory-name").text(pck_name);
	}else{
	  $("#subcategory-name").text('Subcategory Name');
	}
});

...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		success: function(data){
			if(data.status == 1){
				swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=base_url('admin/subscription')?>"});
			}
			if(data.status == 0){
...
$(document.body).on('click', '#product_category' ,function(){ 
		...
		$.ajax({      
		url: '<?=base_url('admin/product/getSub')?>',       
		type: 'POST',            

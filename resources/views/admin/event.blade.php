                  ...
                  <div class="card-body">
                  	<div class="row">
	                     ...
	                     <div class="col-sm-2 text-end" style="padding-left: 54px;">
	                     	<a href="<?=url('admin/event/add')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
	                     </div>
	                  </div>   	
                     <div class="">
					    @if (session('status'))
                        ...
                        <table id="datatable" class="table table-bordered dt-responsive nowrap w-100">
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $k => $v): ?>
											<tr>
												<td><?= $k+1 ?></td>
												<td><?=substr(@$v->event_name,0,30)?></td>
												<td><?=substr(strip_tags(@$v->description),0,40)?></td>
												<?php
												    if(empty($v->user_id) || $v->user_id == 0){
														$username = 'Admin';
													}else{
														$userType = DB::table('users')->where(['id' => @$v->user_id])->select('first_name', 'last_name')->first();
														$username = @$userType->first_name.' '.@$userType->last_name;
													}
												?>
												<td><?=@$username ?></td>
												
												<td><?=date('M d, Y h:i A', strtotime(@$v->start_date)); ?></td>
												<td>
												...
												<td class="text-center">
													<a href="<?= url('admin/event/edit/'.$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
													    <i class="fas fa-edit"></i>
													</a>  
													
													
													
												    <a href="<?= url('admin/event/view/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="View">
													    <i class="fa fa-eye"></i>
													</a>
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/event/delete-event/') ?>/'+dealId
			}
		});
	}
	...
	function changeDealStatus(id, thisSwitch) {      
		...
		$.ajax({         
			url: '<?php echo url('admin/event/changestatus'); ?>',     
			type: 'POST',       
			dataType: 'json',       
			data: {         
				id: String(id),        

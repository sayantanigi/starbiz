								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $k => $v): ?>
											<tr>
												...
												<td><?=@$v->phone; ?></td>
												<?php
												    $userType = DB::table('user_type')->where(['id' => @$v->user_type])->select('name')->first();
												?>
												<td><?=@$userType->name; ?></td>
												<td>
												...
												<td class="text-center">
													...
													<a href="<?= url('admin/users/subscription/'.@$v->id) ?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Subscription Payment">
													    Payment
													</a>
													<?php  if(@$userMenu_1->deleted == 1){?>

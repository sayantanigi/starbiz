										...
										<?php foreach ($result as $k => $v): ?>
											<tr id="remove_row_<?=$v->id?>">
												...
												<td>
													...
													<a class="btn btn-info btn-sm" href="<?= url('admin/emailtemplate/update/'.base64_encode($v->id)); ?>">
														<i class="far fa-edit">
														</i>
														Edit
													</a>
													
													<a class="btn btn-danger btn-sm" title="Delete"  onclick="deleteSendMail(<?=$v->id?>)">
...
function deleteSendMail(id)
{
	...
	if (isConfirm) {
			$.ajax({
				type: "POST", 
				url:  '<?= url('admin/emailtemplate/delete_template/') ?>',  

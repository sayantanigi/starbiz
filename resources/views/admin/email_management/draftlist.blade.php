												...
												<tr id="remove_row_<?=$v->id?>">
													...
													<?php
													    if(@$v->created_date != '0000-00-00 00:00:00'){
															echo date('d M, Y', strtotime(@$v->created_date));
														}else{
															echo '';
														}
													?>
													...
													<td>
                                                        <a href="<?=url('admin/mailer/update?dId='.base64_encode(@$v->id).'')?>"  title="Edit" ><i class="far fa-edit"></i> </a>	
												    </td>
...
function get_recipients(id){
	$.ajax({
		type: "POST", 
		url:  '<?= url('admin/mailer/get_recipients/') ?>',  
...
function deleteSendMail(id)
{
	...
	if (isConfirm) {
			$.ajax({
				type: "POST", 
				url:  '<?= url('admin/mailer/delete_send_list/') ?>',  
				data: {id:id}, 
				dataType : 'json',
				beforeSend: function(){
				},

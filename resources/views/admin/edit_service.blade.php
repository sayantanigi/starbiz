                     ...
                     <div class="card-body">    
                        <form action="{{url('admin/services/update')}}" method="post" enctype="multipart/form-data" >
                            ...
                            <div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Service Name *</label>
                                <input type="text" class="form-control" name="service_name"  id="service_name"  autocomplete="off" value="<?=@$result->name?>" required >
                            </div>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Category *</label>
                                
								<select  class="form-control" name="service_category"  id="service_category"  autocomplete="off" required>
								    <option value="">Select Category</option>
									<?php
										if(@$category){
											foreach(@$category as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$result->category == @$v->id) ? 'selected' : '').'>'.@$v->name.'</option>';
											}
										}
									?>
								</select>
							...
							<!--<div class="form-group mb-2">
								...
								<select  class="form-control" name="product_subcategory"  id="product_subcategory"  autocomplete="off" required>
								    <option value="">Select Subcategory</option>
									<?php
									    if(@$subcat){
											foreach(@$subcat as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$v->id == @$result->subcategory) ? 'selected' : '').'>'.@$v->name.'</option>';
											}
										}
									?>
								</select>
							...
							<div class="form-group mb-2">
								...
								<select  class="form-control" name="service_listing"  id="service_listing"  autocomplete="off" required>
								    <option value="">Select Listing</option>
									<?php
										if(@$listing){
											foreach(@$listing as $k => $v){
												echo '<option value="'.@$v->id.'" '.((@$result->listing_id == @$v->id) ? 'selected' : '').'>'.@$v->business_name.'</option>';
											}
										}
									?>
								</select>
							...
							<!--<div class="form-group mb-2">
                                ...
                                <input type="number" class="form-control" name="product_quantity"  id="product_quantity" value="<?=@$result->quantity?>" autocomplete="off" required >
                            </div>
							...
							<div class="form-group mb-2">
                                ...
                                <select class="form-control" name="product_availability"  id="product_availability" required>
                                    <option value="1" <?=((@$result->availability == 1) ? 'selected' : '')?>>In Stock</option>
							...
							<div class="form-group mb-2">
                                ...
                                <textarea type="text" class="form-control editor summermote" name="service_description"  id="service_description"  autocomplete="off"  required><?=@$result->description?></textarea>
                            </div>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Service Image </label>
                                <input type="file" class="form-control" name="service_image[]"  id="service_image"  autocomplete="off" multiple>
                                <input type="hidden" class="form-control" name="service_id"  id="service_id"   value="<?=@$result->id?>">
                            </div>
							<?php
							    $image = DB::table('services_image')->where(['service_id' => @$result->id])->select('*')->orderBy('id', 'DESC')->get();
							?>
								    ...
								    if($image){
										foreach($image as $k => $v){
											...
											if(!empty(@$v->image) && file_exists('public/service/'.@$v->image.'')){
												$productImg = url('service/'.@$v->image.'');
											}else{
												$productImg = '';
											}
											
											echo '<div class="uploadedimg preview-show-'.@$v->id.'" id="preview'.@$v->id.'"><img src="'.@$productImg.'">
											<a  href="javascript:void(0);" class="closeimg image-cancel-1 removeImg" data-no="'.@$v->id.'"  id="img'.@$v->id.'"><i class="fa fa-times" style="color: black;"></i></a>
											</div>';
										}
									}
								?>
...
$(document).ready(function(e){
	$(document).on('change','#service_image',function(e){
		...
		var totalfiles = document.getElementById('service_image').files.length;
				...
				for (var index = 0; index < totalfiles; index++) {
					dataTransfer.items.add(document.getElementById('service_image').files[index]);
					var fileType = document.getElementById('service_image').files[index].type;
					if(!((fileType == match[0]) || (fileType == match[1]) || (fileType == match[2]) || (fileType == match[3]) || (fileType == match[4]))){
						...
						var fileName = document.getElementById('service_image').files[index].name;
...
$('#tags').chosen({max_selected_options:10,width:'100%'});
 ...
 $(document).on('keyup','#name',function(e){
	...
	$(document.body).on('click', '.removeImg' ,function(){ 
		var item = $(this).attr('data-no');
		
		$.ajax({      
		url: '<?=url('admin/services/removeImg')?>',       
		type: 'POST',            

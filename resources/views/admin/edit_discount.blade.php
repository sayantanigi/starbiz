            ...
            <div class="row">
                ...
                <div class="col-lg-6 mb-3">
                  <div class="card shadow rounded">
                     <div class="card-body">    
                        <form action="{{url('admin/discount/update')}}" method="post" enctype="multipart/form-data" >
                             @csrf
                            <div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Select Type</label>
                                <select class="form-control" name="type"  id="type" required>
                                    <option value="">Select Type</option>
                                    <option value="0" <?=((@$result->type == 0) ? 'selected' : '')?>>Discount In Amount</option>
                                    <option value="1" <?=((@$result->type == 1) ? 'selected' : '')?>>Discount In Percentage</option>
                                </select>
								<span id="type_error" class="text-danger"></span>
                            </div>
							
							<div class="form-group mb-2" style="display:<?=((@$result->type == 1) ? 'block' : 'none')?>;" id="discountPer">
                                <label class="fw-semibold  text-black">Discount Percentage</label>
                                <input type="number" class="form-control" name="percentage"  id="percentage" autocomplete="off" value="<?=@$result->discount?>">
                                <input type="hidden" class="form-control" name="id"  id="id"  value="<?=@$result->id?>">
								<span id="discount_error" class="text-danger"></span>
                            </div>
							
							<div class="form-group mb-2" style="display:<?=((@$result->type == 0) ? 'block' : 'none')?>;" id="amountcountPer">
                                <label class="fw-semibold  text-black">Discount Amount</label>
                                <input type="number" class="form-control" name="amount"  id="amount" autocomplete="off" value="<?=@$result->discount?>">
								<span id="discount_error" class="text-danger"></span>
                            </div>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Coupon Code</label>
                                <input type="text" class="form-control" name="promo_code"  id="promo_code"  autocomplete="off" required value=<?=@$result->coupon_code?>>
								<span id="promo_code_error" class="text-danger"></span>
                            </div>
							<?php
							    if(!empty(@$result->expire_on) || (@$result->expire_on != '0000-00-00')){
									$expire_on = date('Y-m-d', strtotime(@$result->expire_on));
								}else{
									$expire_on = '';
								}
							?>
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Expire On</label>
                                <input type="date" class="form-control" name="expire_on"  id="expire_on"  autocomplete="off" required value="<?=@$expire_on?>">
								<span id="expire_on_error" class="text-danger"></span>
                            </div>
                       ...
                       <div class="card-body">
                            <div class="row">
								 ...
								 <div class="col-lg-12 mb-3">
									...
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Type</h6></label><p class="text-muted" id="text_discount"><?=@$type?></p></div>
									<?php
										if(@$result->type == 1){
									...
									?>
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Coupon Code</h6></label><p class="text-muted" id="promocode"><?=@$result->coupon_code?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6><?=@$text?></h6></label><p class="text-muted" id="disper"><?=@$discount?></p></div>
									
									<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Status</h6></label><p class="text-muted" id="coach_status"><?=@$status?></p></div>
									</div>
									</div>
                                </div>
                            </div>
...
$(document).on('change','#type',function(e){
	...
	//alert(type);
	if(type == '1'){
		$("#discountPer").css('display', 'block');
		$("#amountcountPer").css('display', 'none');
		$("#text_discount").text('Discount In Percentage');
	}
	if(type == '0'){
		$("#discountPer").css('display', 'none');
		$("#amountcountPer").css('display', 'block');
		$("#text_discount").text('Discount In Amount');
	}
	if (type == " "){
		$("#discountPer").css('display', 'none');
		$("#amountcountPer").css('display', 'none');
...
$(document).ready(function(){
	$("#submitform").on('submit', function(e){
		...
		form_data.append("status", userstatus);
		form_data.append("expire_on", expire_on);
		
		$.ajax({
		type: 'POST',
		url: '<?php echo url('admin/users/adddis'); ?>',
		data: form_data,
		dataType:"json",
		contentType: false,
		cache: false,
		...
		success: function(data){
			if(data.status == 1){
				swal({title: "Sucess!", text: "<strong>"+data.message+"</strong>", type: "success", showConfirmButton: true, html:true}, function(){ window.location.href = "<?=url('admin/users/discount')?>"});
			...
			if(data.vali_error == 1){
				...
				if(data.promo_code_error != ''){
					$('#promo_code_error').html(data.promo_code_error);
				}else{
				...
				if(data.expire_on_error != ''){
					$('#expire_on_error').html(data.expire_on_error);
				}else{
					$('#expire_on_error').html('');
				}
	...
	$(document).on('keyup','#percentage',function(e){
		...
		if(lname){
		    ...
		    $("#disper").text('Discount Percentage');
		}
	});

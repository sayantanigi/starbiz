								...
								<div class="container">
									<div class="col-md-12">
										<div class="profile clearfix">
                                            <?php
											    $image1 = DB::table('listing_image')->where(['listing_id' => @$result->id])->select('*')->orderBy('id', 'ASC')->first();
										    ?>										
											<div class="image item" id="Cover-Image">
											    <img src="<?= ((!empty(@$image1->image) && file_exists('public/listing/'.@$image1->image.'')) ? url('listing/'.@$image1->image.'') : url('events/bnr.jpg')); ?>" class="img-cover">
											</div>                            
                                ...
                                <div class="col-lg-12 mb-3">
									<div class="card rounded">
										<div class="card-body">
											...
											?>
											<div class="mt-3"> <label class="tx-11 font-weight-bold mb-0 "><h6>Business Name</h6></label><p class="text-muted" id="game_name"><?=@$business_name?></p></div>
											...
											<?php
												$category = DB::table('listing_category')->where(['id' => @$result->category])->select('*')->orderBy('id', 'DESC')->first();
											?>
											...
											<?php
											    ...
											    if(@$result->subcategory){
												    $subcategory = DB::table('listing_sub_category')->where(['id' => @$result->subcategory])->select('*')->orderBy('id', 'DESC')->first();
											...
											<!--<div class="row">
												<div class="col-md-6">
													<?php
													    $country = DB::table('countries')->where(['id' => @$result->country])->select('*')->orderBy('id', 'DESC')->first();
													?>
													<label class="tx-11 font-weight-bold mb-0 "><h6>Country</h6></label><p class="text-muted" id="game_description"><?=@$country->name; ?></p>
												</div>
												
												    <?php
													    $states = DB::table('states')->where(['id' => @$result->state])->select('*')->orderBy('id', 'DESC')->first();
													?>
												
												<div class="col-md-6">
													<label class="tx-11 font-weight-bold mb-0 "><h6>State</h6></label><p class="text-muted" id="game_description"><?=@$states->name; ?></p>
												</div>
												    <?php
													    $cities = DB::table('cities')->where(['id' => @$result->city])->select('*')->orderBy('id', 'DESC')->first();
													?>
												<div class="col-md-6">
													<label class="tx-11 font-weight-bold mb-0 "><h6>City</h6></label><p class="text-muted" id="game_description"><?=@$cities->name; ?></p>
												</div>
												
											</div>-->
											   
											    <!--<div class="col-md-6">
													<label class="tx-11 font-weight-bold mb-0 "><input type="radio"  value="1"  name="online_business1"  style="width: 20px;height: 20px;" <?=((@$result->online_busi == '1') ? 'checked' : '')?>> &nbsp; <h6 style="position: absolute;margin: -25px 36px;"> Online Business</h6></label></p>
												</div>-->
												
												<!--<div class="row">
													<div class="col-md-6">
														
														<label class="tx-11 font-weight-bold mb-0 "><input type="radio"  value="1"  name="online_business2"  style="width: 20px;height: 20px;" <?=((@$result->google_map_address == 0) ? 'checked' : '')?>> &nbsp;<h6 style="position: absolute;margin: -25px 36px;">Do not have Google Map Address?</h6></label><p class="text-muted" id="game_description"></p>
													</div>
													
													
													<div class="col-md-6">
														
														<label class="tx-11 font-weight-bold mb-0 "><input type="radio"  value="1"  name="online_business2"  style="width: 20px;height: 20px;" <?=((@$result->google_map_address == 1) ? 'checked' : '')?>> &nbsp;<h6 style="position: absolute;margin: -25px 36px;">I have Google Map Address?</h6></label><p class="text-muted" id="game_description"></p>
													</div>
											...
											<div class="row">
												<div class="col-md-12">
													...
													<label class="tx-11 font-weight-bold mb-0 "><h6>Address</h6></label><p class="text-muted" id="game_description"><?=@$result->address; ?></p>
												</div>
												
												   
												
												<div class="col-md-6">
													<label class="tx-11 font-weight-bold mb-0 "><h6>Latitude</h6></label><p class="text-muted" id="game_description"><?=@$result->latitude; ?></p>
												</div>
												   
												<div class="col-md-6">
													<label class="tx-11 font-weight-bold mb-0 "><h6>Longitude</h6></label><p class="text-muted" id="game_description"><?=@$result->longitude; ?></p>
												</div>
												
												<div class="col-md-12">
													
													<label class="tx-11 font-weight-bold mb-0 "><h6>Phone</h6></label><p class="text-muted" id="game_description"><?=@$result->phone; ?></p>
												</div>
												
												<div class="col-md-6">
													
													<label class="tx-11 font-weight-bold mb-0 "><h6>Email</h6></label><p class="text-muted" id="game_description"><?=@$result->email; ?></p>
												</div>
												
												<div class="col-md-6">
													
													<label class="tx-11 font-weight-bold mb-0 "><h6>Website</h6></label><p class="text-muted" id="game_description"><?=@$result->website; ?></p>
												</div>
										...
										<div class="card-body">
											...
											<div class="row">
												<div class="eventphoto my-3 specific_preview">
													...
													<?php
													$image = DB::table('listing_image')->where(['listing_id' => @$result->id])->select('*')->orderBy('id', 'ASC')->get();
													?>

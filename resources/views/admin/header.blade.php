...
<head>
    ...
    <link href="<?= url('assets/admin/plugins/admin-resources/jquery.vectormap/jquery-jvectormap-1.2.2.css') ?>"
        rel="stylesheet" type="text/css" />
    <link href="<?= url('assets/admin/dist/css/bootstrap.min.css') ?>" id="bootstrap-style" rel="stylesheet"
        type="text/css" />
    <link href="<?= url('assets/admin/dist/css/icons.min.css') ?>" rel="stylesheet" type="text/css" />
    <link href="<?= url('assets/admin/plugins/datatables.net-bs4/css/dataTables.bootstrap4.min.css') ?>"
        rel="stylesheet" type="text/css" />
    <link href="<?= url('assets/admin/plugins/datatables.net-buttons-bs4/css/buttons.bootstrap4.min.css') ?>"
        rel="stylesheet" type="text/css" />
    <link href="<?= url('assets/admin/plugins/datatables.net-select-bs4/css/select.bootstrap4.min.css') ?>"
        rel="stylesheet" type="text/css" />
    <link href="<?= url('assets/admin/plugins/jasny-bootstrap/jasny-bootstrap.min.css') ?>" rel="stylesheet"
        type="text/css" />
    ...
    <link href="<?= url('assets/admin/plugins/bootstrap-datepicker/css/bootstrap-datepicker.min.css') ?>"
        rel="stylesheet">
    ...
    <div id="layout-wrapper">
                  ...
                  <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                    ...
                    <div class="dropdown d-inline-block">
                        <button type="button" class="btn header-item noti-icon waves-effect"
                            ...
                            <i class="ri-notification-3-line"></i>
                            <!-- Display unread count as badge -->
                            <span class="noti-dot" id="notification-count-badge" style="display:none;"></span>
                            <!-- Initially hidden -->
                        </button>
                        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-end p-0"
                            aria-labelledby="page-header-notifications-dropdown">
                            <div class="p-3">
                                <div class="row align-items-center">
                                    ...
                                    <div class="col-auto">
                                       <a href="{{ url('/admin/notifications/' . 1) }}" class="small">View All</a>
                            ...
                            <!-- Notifications List -->
                            <div id="notifications-list" class="px-3" style="max-height: 300px; overflow-y: auto;">
                    ...
                    <div class="dropdown d-inline-block user-dropdown">
                        <button type="button" class="btn header-item waves-effect" id="page-header-user-dropdown"
                            ...
                            <img class="rounded-circle header-profile-user"
                                src="<?= url('setting/' . @$settings->favicon . '') ?>" alt="Header Avatar">
                        </button>
        ...
        <div class="vertical-menu">
            ...
            <div class="navbar-brand-box">
                ...
                <a href="" class="logo logo-light">
                    <span class="logo-sm">
                        <img src="<?= url('setting/' . @$settings->favicon . '') ?>" alt="" height="22"
                            style="height: 70px;width: 170%;object-fit: contain;">
                    </span>
                    <span class="logo-lg">
                        <img src="<?= url('setting/' . @$settings->logo . '') ?>" alt="" height="22"
            ...
            <script>
                $(document).ready(function() {
                    // alert(213);
                    const adminId = 1; // Example admin ID, you can dynamically set it based on logged-in admin

                    // Fetch unread notification count
                    function fetchUnreadCount() {
                        var baseUrl = "<?= url('admin/notification/getUnreadCount') ?>/" + adminId;
                        $.ajax({
                            url: baseUrl,
                            type: 'GET',
                            ...
                            success: function(data) {
                                ...
                                if (unreadCount > 0) {
                                    $('#notification-count-badge').show(); // Show the notification badge
                                    $('#notification-count-badge').text(unreadCount); // Update the badge count
                                } else {
                                    $('#notification-count-badge')
                                        .hide(); // Hide the badge if no unread notifications
                                }
                            }
                        });
                    }

                    // Fetch all notifications (called when user clicks "View All")
                  //   function fetchAllNotifications() {
                  //       var baseUrl = "<?= url('admin/notifications/all') ?>/" + adminId;
                  //       $.ajax({
                  //           url: baseUrl,
                  //           type: 'GET',
                  //           dataType: 'json',
                  //           success: function(data) {
                  //               $('#notifications-list').empty(); // Clear the list

                  //               if (data.length === 0) {
                  //                   $('#notifications-list').append('<p>No notifications available.</p>');
                  //               } else {
                  //                   data.forEach(function(notification) {
                  //                       $('#notifications-list').append(`
                  //                <div class="notification-item">
                  //                   <strong>${notification.sender_id}</strong>: ${notification.noti_msg}
                  ...
                  //   });
                    ...
                    //     $.ajax({
                    //         url: 'path/to/your/controller/NotificationController.php?action=sendNotification',
                    //         type: 'POST',
                    //         data: {
                    //             sender_id: adminId,
                    //             receiver_id: 2, // Store ID, replace with actual store ID
                    //             message: message
                    //         },
                    //         dataType: 'json',
                    //         success: function (data) {

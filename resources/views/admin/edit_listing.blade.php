                     ...
                     <div class="card-body">    
                        <form action="{{url('admin/listing/update')}}" method="post" enctype="multipart/form-data" >
                            ...
                            <div class="form-group mb-2">
                                ...
                                <input type="text" class="form-control" name="listing_business_name"  id="listing_business_name"  autocomplete="off" value="<?=@$result->business_name?>" required>
                                <input type="hidden" class="form-control" name="listing_id"  id="listing_id"  value="<?=@$result->id?>">
                            </div>
							<small id="pck_listing_business_name"></small>
							
							<div class="form-group mb-2">
                                <label class="fw-semibold  text-black">Name </label>
								 <input type="text" class="form-control" name="listing_name"  id="listing_name"  autocomplete="off" value="<?=@$result->name?>">
								...
								<div class="row">
										   ...
										   <?php
											    if(!empty(@$country)){
													foreach(@$country as $k => $v){
														echo '<option value="'.$v->id.'" '.((@$v->id == @$result->country) ? 'selected' : '').'>'.$v->name.'</option>';
													}
												}
											?>
									...
									<label class="fw-semibold  text-black">State *</label>
								        <select class="form-control" name="state" id="statelist" onchange="getlistcity($(this).val());" required>
											...
											<?php 
											    $statelist = DB::table('states')->where(['country_id' => @$result->country])->select('*')->orderBy('name', 'ASC')->get();
												foreach($statelist as $k => $v){
													echo '<option value="'.@$v->id.'" '.((@$v->id == @$result->state) ? 'selected' : '').'>'.@$v->name.'</option>';
												}
											?>
									    </select>
								    </div>
								...
								<div class="row">
									...
									<label class="fw-semibold  text-black">City *</label>
									    <select class="form-control" name="city" id="citylist" required>
											...
											<?php 
											    $citylist = DB::table('cities')->where(['state_id' => @$result->state])->select('*')->orderBy('name', 'ASC')->get();
												foreach($citylist as $k => $v){
													echo '<option value="'.@$v->id.'" '.((@$v->id == @$result->city) ? 'selected' : '').'>'.@$v->name.'</option>';
												}
											?>
									    </select>
								    </div>-->
											...
											<?php
											    if(!empty(@$subcat)){
													foreach(@$subcat as $k => $v){
														echo '<option value="'.@$v->id.'" '.((@$result->subcategory == @$v->id) ? 'selected' : '').'>'.@$v->name.'</option>';
													}
												}
											?>
									    </select>
							...
							<div class="form-group mb-2">
								<div class="row">
								    <div class="col-sm-6">
										...
										<input type="radio"  value="1"  name="online_business"  style="width: 20px;height: 20px;" <?=((@$result->online_busi == 1) ? 'checked' : '')?>> &nbsp;  <span>Online Business</span></label>
								    </div>
								</div>
                            </div>
							...
							<!--<div class="form-group mb-2">
								<div class="row">
								    <div class="col-sm-6">
										<label class="fw-semibold  text-black"> 
										<input type="radio" class="chkReason" id="chkmap" value="0"  name="chk_address"  style="width: 20px;height: 20px;" <?=((@$result->google_map_address == 0) ? 'checked' : '')?>> &nbsp;  <span>Do not have Google Map Address?</span></label>
								    </div>
									
									 <div class="col-sm-6">
										<label class="fw-semibold  text-black"> 
										<input type="radio" class="chkReason" id="chkgooglmap" value="1"  name="chk_address"  style="width: 20px;height: 20px;" <?=((@$result->google_map_address == 1) ? 'checked' : '')?>>&nbsp;  <span>I have Google Map Address?</span></label>
								    </div>
								</div>
                            </div>-->
							
							
							<!--<div class="form-group mb-2" id="mannual_block" style="display: <?=((@$result->google_map_address == 0) ? 'block' : 'none')?>">
								<div class="col-sm-12">
							...
							<div class="form-group mb-2" id="auto_block" >
								<div class="col-sm-12">
									<label class="fw-semibold  text-black">Address </label>
									<input type="text" name="google_address" id="google_address" class="form-control" value="<?=@$result->address?>">
									<input type="hidden" name="google_latitude" id="google_latitude" class="form-control" value="<?=@$result->latitude?>">
									<input type="hidden" name="google_longitude" id="google_longitude" class="form-control" value="<?=@$result->longitude?>">
								</div>
                            </div>
							
							<!--<div class="form-group mb-2" id="auto_block" style="display: <?=((@$result->google_map_address == 1) ? 'block' : 'none')?>">
								<div class="col-sm-12">
									<label class="fw-semibold  text-black">Address </label>
									<input type="text" name="google_address" id="google_address" class="form-control" value="<?=@$result->address?>">
									<input type="hidden" name="google_latitude" id="google_latitude" class="form-control" value="<?=@$result->latitude?>">
							...
							<div class="form-group mb-2">
								...
								<div class="row">
									...
									<label class="fw-semibold  text-black">Phone *</label>
									    <input  type="number" class="form-control" name="phone" id="phone" required value="<?=@$result->phone?>">

								    </div>
								    <div class="col-sm-6">
									<label class="fw-semibold  text-black">Email *</label>
								        <input type="email" class="form-control" name="email" id="email" required value="<?=@$result->email?>">
								    </div>
								</div>
                            </div>
							...
							<div class="form-group mb-2">
								...
								<div class="row">
								    <div class="col-sm-12">
									    ...
									    <input type="text" class="form-control" name="website" id="website" value="<?=@$result->website?>">
								    </div>
								</div>
                            </div>
							...
							<div class="form-group mb-2">
								...
								<div class="eventphoto my-3 specific_preview">
								    <?php
									    $imagelist = DB::table('listing_image')->where(['listing_id' => @$result->id])->select('*')->get();
								        if(!empty(@$imagelist)){
										    foreach(@$imagelist as $k => $v){
											   echo '
													<div class="uploadedimg preview-show-'.@$v->id.'" id="preview'.@$v->id.'">
													<img src="'.(!empty($v->image) ? url('listing/'.$v->image.'') : '').'">
...
$(document.body).on('click', '.removeImage' ,function(){ 
		...
		$.ajax({
			url: "<?php echo url('admin/listing/deleteGallery'); ?>",
			type: "POST",
			data: {

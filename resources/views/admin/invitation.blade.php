<?php
   $userMenu_1 = DB::table('role_permission')->where(['role_id' => session()->get('ROLE_ID'), 'menu_id' => 6])->select('*')->orderBy('id', 'DESC')->first();
?>
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $k => $v): ?>
											<tr>
												<td><?= $k+1 ?></td>
												<?php
													$senderInfo = DB::table('users')->where(['id' => @$v->sender_id])->select('first_name', 'last_name')->first();
													$receiverInfo = DB::table('users')->where(['id' => @$v->receiver_id])->select('first_name', 'last_name')->first();
												...
												<td>
													<?php
														...
														echo '<span style="background: '.$bgcolor.';color: #fff;padding: 3px 13px;font-size: 14px;font-weight: 700;border-radius: 4px;">'.$status.'</span>';
													?>
												</td>

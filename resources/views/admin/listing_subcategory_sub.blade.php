                  	...
                  	<div class="row">
	                  	<div class="col-sm-10">
							 ...
							 <div class="col-sm-2 text-end" style="padding-left: 54px;">
								 <a href="<?=url('admin/listing/add-subcategory')?>" class="btn btn-success btn-sm"><i class="fa fa-plus"></i>&nbsp;Add</a>
							 </div>
								   ...
								   <?php if (is_array($result) || is_object($result)) { ?>
										<?php foreach ($result as $key => $v): ?>
											<tr>
												<td><?= $key+1 ?></td>
												<?php
												    $cate = DB::table('listing_sub_category')->where(['id' => @$v->listing_subcategory_id])->select('*')->orderBy('id', 'DESC')->first();
												?>
												<td><?= @$cate->name?></td>
												<td><?= @$v->name?></td>
												...
												<td class="text-center">
														...
														<a href="<?=url('admin/listing/edit-subcategory-sub/'.@$v->id.'')?>" class="btn btn-outline-success btn-sm" data-toggle="tooltip" title="Edit">
   ...
   function dealDetail(dealId) {
	...
	function deleteDeals(dealId) 
	{
		swal({
			...
			if (isConfirm) {
				window.location.href = '<?= url('admin/listing/delete-subcategory') ?>/'+dealId
			}
		});
	}

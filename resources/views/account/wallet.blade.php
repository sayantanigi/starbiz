...
<style>
    ...
    <div class="container-fluid m-0 Section Wallet" >
      <!-- Add Top Up Modal -->
      <div class="modal fade CustomModal" id="AddTopUpModal" data-bs-backdrop="static" data-bs-keyboard="false"
        ...
        <div class="modal-dialog modal-dialog-centered modal-sm">
          <div class="modal-content">
            ...
            <div class="modal-body">
              <form class="row g-3" id="topUpPay">
           ...
           <!-- Withdraw Modal -->
      <div class="modal fade CustomModal" id="WithdrawModal" data-bs-backdrop="static" data-bs-keyboard="false"
        tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
          <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
              ...
              <form class="row g-3" id="withdrawFormAmount">
                <div class="col-md-12 col-sm-12">
                  <label class="form-label">Enter Amount</label>
                  <input type="text" placeholder="Enter amount" id="withdrawAmount" name="withdrawAmount">
                </div>
				
				<div class="col-md-12 col-sm-12">
                  <label class="form-label">Bank Name</label>
                  <input type="text" placeholder="Enter Bank Name" id="bankName" name="bankName">
                </div>
				
                <div class="col-md-12 col-sm-12">
                  <label class="form-label">Account Number</label>
                  <input type="number" placeholder="Enter Account Number" id="withdrawAccountno" name="withdrawAccountno">
                </div>
				
				<div class="col-md-12 col-sm-12">
                  <label class="form-label">Swift Code</label>
                  <input type="text" placeholder="Enter Swift Code" id="swiftCode" name="swiftCode">
                </div>

				<div class="modal-footer">
				    <button type="submit" class="btn btn-primary w-100">Withdraw Amount</button>
				</div>
              </form>
      ...
      <div class="row m-0 TabBar mb-2">
        <div class="Pagination TabBar">
          <a href="<?=url('dashboard')?>" ><i class="fa fa-angle-left" aria-hidden="true"></i> Home / Wallet</a>
        </div>
        ...
        <div class="row m-0 align-items-center d-flex flex-row justify-content-center" style="background: #fff;">
          <div class="col-lg-3 col-md-3 col-sm-12 ReferralDataContainer">
            <p class="m-0 Heading">Main Balance</p>
            <p class="m-0 SubHeading">$<?=number_format(@$walletAmount->wallet_amount,2);?></p>
          </div>
          <div class="col-lg-2 col-md-2 col-sm-12 WalletBtnContainer">
            <a href="" class="WalletBtnBlock" data-bs-toggle="modal" data-bs-target="#AddTopUpModal">
              <img src="<?=url('assets/home/images/Icon36.png')?>" alt="">
              <p class="m-0">Top Up</p>
            </a>
            <a href="" class="WalletBtnBlock" data-bs-toggle="modal" data-bs-target="#WithdrawModal">
              <img src="<?=url('assets/home/images/Icon37.png')?>" alt="">
              <p class="m-0">Withdraw</p>
            </a>
          </div>
		  
		  <div class="col-lg-3 col-md-3 col-sm-12 ExportBtnContainer" style="    display: flex;
    align-items: center;
    justify-content: center;">
            <a href="<?=url('dashboard/downloadCsvWallet')?>" class="ExportBtn" style="box-shadow: 0 0 10px #ddd;
    background: #fff;
    display: flex;
    align-items: center;
    justify-content: center;
    ...
    font-size: 16px;
    font-weight: 600;
    color: #c5a668;">Export</a>
          </div>
        </div>
        <!--<a href="<?=url('dashboard/downloadCsvWallet')?>" class="btn btn-primary" style="background: #c5a668;
    color: #fff;
    border-color: #c5a668;
    position: absolute;
    margin: 225px 75rem;
    width: 10%;">Export</a>-->
		...
		<div class="TabContainer">
			<div class="Tab active" onclick="openTab(event, 'AllTransaction')">All Transaction</div>
			<div class="Tab" onclick="openTab(event, 'TopUp')">Top Up</div>
			<div class="Tab" onclick="openTab(event, 'Withdraw')">Withdraw</div>
		</div>
		
      </div>

      <div id="AllTransaction" class="row m-0 TabContent active">
	    <?php
		    if(count(@$alltranList) > 0){
				foreach(@$alltranList as $k => $v){
					...
					echo '
						<div class="col-lg-4 col-md-4 mb-4">
						  <div class="TransactionBlock">
							...
							<p class="TransactionAmount '.@$Red.'">$'.@$v->amount.'</p>
						  </div>
						</div>
					';
				}
      ...
      <div id="TopUp" class="row m-0 TabContent">
	    <?php
		    if(count(@$toptranList) > 0){
				foreach(@$toptranList as $k => $v){
      ...
      <div id="Withdraw" class="row m-0 TabContent">
...
$(document).ready(function(){
	...
	$("#topUpPay").on('submit', function(e){
        ...
        window.location.href = "<?=url('dashboard/topup-payment-page/?')?>userId="+userId+"&amount="+amount+"";
	});
	
	
	$("#withdrawFormAmount").on('submit', function(e){
		e.preventDefault();	
		var form_data = new FormData();
         
		var withdrawAmount   = $('#withdrawAmount').val(); 
		var withdrawAccountno   = $('#withdrawAccountno').val(); 
		var bankName   = $('#bankName').val(); 
		var swiftCode   = $('#swiftCode').val(); 
		var userId   = '<?=session()->get('USERLOGINID')?>';

		form_data.append("withdrawAmount", withdrawAmount);
		form_data.append("withdrawAccountno", withdrawAccountno);
		form_data.append("bankName", bankName);
		form_data.append("swiftCode", swiftCode);
		...
		$.ajax({
			...
			type: 'POST',
			url: '<?php echo url('dashboard/withdrawAmount'); ?>',
			data: form_data,
			dataType:"json",
			contentType: false,
			cache: false,

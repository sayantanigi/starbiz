    ...
    <main role="main" class="Main">
      ...
      <div class="row m-0 TabBar mb-2">
        <div class="Pagination TabBar">
          <a href="<?=url('dashboard')?>" id="Home"><i class="fa fa-angle-left" aria-hidden="true"></i> Home / Transactions & Payment</a>
        </div>
      </div>
					...
					if(@$v->product_info){
						...
						foreach($product as $proKey => $proVal){
							...
							if(!empty(@$productInfo)){
								...
								$proImg = DB::table('product_image')->where(['product_id' => $proVal['product_id']])->select('*')->orderBy('id', 'DESC')->first();

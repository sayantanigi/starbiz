				...
				<?php
					if(count(@$result) > 0){
						foreach(@$result as $k => $v){
							...
							$userInfo = DB::table('users')->whereRaw("id=".@$v->id."")->select('*')->orderBy('id', 'DESC')->first();
							...
							$numRows = DB::table('favouriteusers')->where(['user_id' => session()->get('USERLOGINID'), 'fav_user_id' => @$userInfo->id])->select('*')->orderBy('id', 'DESC')->count();
							...
							echo '
								<div class="Card col-lg-4 col-md-4 col-sm-6 mt-3 post-item" relid-1="'.@$userInfo->id.'">
								  <div class="NetworkCartBlock" >
									<a href="'.url('dashboard/network-details/'.@$userInfo->id.'').'"><div class="UserDetails NetworkProfile-11" relid="'.@$userInfo->id.'">
									  <img src="'.@$profilePic.'" alt="" >
									  <p class="Heading" style="color:black;">'.@$userInfo->first_name.' '.@$userInfo->last_name.'</p>
									</div></a>
			...
			<div id="MyAllBusiness" class="row m-0 TabContent active">
				<?php
					if(@$result){
						...
						$image    = DB::table('listing_image')->where(['listing_id' => @$listingInfo->id])->select('*')->orderBy('id', 'DESC')->first();
						
						if(@$v->user_id == 0){
							$userName = 'Admin';
							$userProfile = url('profile/unnamed.jpg');
						}else{
							$checkUser = DB::table('users')->where(['id' => @$listingInfo->user_id])->select('*')->orderBy('id', 'DESC')->first();
							if($checkUser){
						...
						if(@$businessMng->write_access == 1 || @$businessMng->full_access == 1){
							$edit = '<a  href="javascript::void(0);" class="edit-business-detail" relid="'.@$listingInfo->id.'" data-bs-toggle="modal" data-bs-target="#EditBusinessModal">
						...
						if(@$businessMng->full_access == 1){						
							$delete = '<a href="javascript:void(0)" onclick="deleteBusiness('. @$listingInfo->id .')" data-bs-toggle="modal" data-bs-target="#DeleteEventModal-1">
						...
						echo '
						<div class="Card col-lg-3 col-md-3 col-sm-6 post-item" relid-1="'.@$listingInfo->id.'">
						  <div class="CardInner business-detail-1"   style="background:url('.@$galleryImg.') no-repeat center center / cover;" relid="'.@$v->id.'" data-bs-toggle="modal" data-bs-target="#DetailsMyBusinessModal">
							...
							<img class="UserImage"
							  src="'.@$userProfile.'"
							  alt="">
							<p class="Heading">'.@$listingInfo->business_name.'</p>
							<p class="SubHeading">'.@$userName.'</p>
							<p class="SubHeading">Location: '.substr(@$listingInfo->address,0,40).'</p>
							<!--<p class="SubHeading">Date: '.@$start_date.'</p>
							<p class="SubHeading">Time: '.@$start_time.'</p>-->
							<div class="IconContainer" >
							
								<a href="javascript:void(0);" class="bookmarkBusiness" id="allbookmarkBusiness_'.@$listingInfo->id.'" relid="'.@$listingInfo->id.'">
									'.@$fav.'
								</a>
...
function getMoreData(lastId) {
      $(window).off("scroll");
    $.ajax({
        url: '<?php echo url('dashboard/load_network_data?lastId='); ?>'+ lastId,
        type: "get",
        beforeSend: function ()
        {
            $('.ajax-loader').show();
